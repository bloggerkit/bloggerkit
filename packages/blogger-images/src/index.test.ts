...
describe('BloggerImage', () => {
  ...
  test('returns s0 when no params remain after clearing', () => {
    const image = new BloggerImage(supportedUrlWithParams).width(null).height(null).size(null).crop(false);
  ...
  test('checks mutually exclusive crop flags and keeps only the last set crop style', () => {
    const image = new BloggerImage(supportedUrlWithParams).crop(true).circularCrop(true).alternateCrop(true);
  ...
  test('sets and removes hex color values correctly', () => {
    const image = new BloggerImage(supportedUrlWithParams).color('0xFFAABB').backgroundColor('0xaabbccdd').padColor('0x000000');
  ...
  test('toggles button and noButton flags with removal behavior', () => {
    const image = new BloggerImage(supportedUrlWithParams).button(true).noButton(true);
